...
- Service ID: &lt;optional, your-service-tree-id&gt;</code></pre>
  ...
  async function fetchPlans() {
    ...
    document
      .querySelectorAll("#tab-release-plans .card-summary.expanded")
      .forEach((el) => {
    ...
    try {
      ...
      if (data.loading) {
        store().loadingMessage =
          "Release plans are being fetched. Dashboard will be available shortly\u2026";
      ...
      if (expandedIds.size) {
        document
          .querySelectorAll("#tab-release-plans .plan-card")
          .forEach((cardEl) => {
            if (expandedIds.has(cardEl.dataset.planId)) {
              ...
              if (summary && details) {
                ...
                if (!summary.dataset.prLoaded) {
                  ...
                  if (cardEl.dataset.planId)
                    lazyLoadPreviousSdkPrs(details, cardEl.dataset.planId);
                }
              }
            }
          });
  ...
  function getPackageFeedInfo(lang) {
    switch (lang) {
      case ".NET":
        return {
          name: "NuGet",
          icon: '<svg class="feed-icon" viewBox="0 0 512 512" width="14" height="14"><circle cx="145" cy="367" r="105" fill="#004880"/><circle cx="371" cy="371" r="105" fill="#004880"/><circle cx="256" cy="145" r="73" fill="#004880"/></svg>',
        };
      case "Python":
        return {
          name: "PyPI",
          icon: '<svg class="feed-icon" viewBox="0 0 512 512" width="14" height="14"><path d="M254 1c-28 0-53 3-74 10-62 19-73 59-73 87v64h147v21H107C62 183 23 212 10 268c-15 64-16 104 0 171 12 49 40 85 88 85h57v-77c0-50 43-94 93-94h146c45 0 81-37 81-82V128c0-44-37-76-81-84-28-5-58-8-86-8h-54zm-82 50c17 0 30 14 30 31s-13 30-30 30c-17 0-31-13-31-30s14-31 31-31z" fill="#366994"/><path d="M393 183v75c0 52-44 96-93 96H154c-44 0-81 38-81 82v154c0 44 53 70 93 82 48 14 94 17 147 0 35-11 68-33 68-82V461H237v-21h215c47 0 64-33 81-82 18-51 17-99 0-171-12-52-35-82-81-82h-59v74zm-55 278c17 0 31 14 31 31s-14 30-31 30-30-13-30-30 13-31 30-31z" fill="#ffc331"/></svg>',
        };
      case "JavaScript":
        return {
          name: "npm",
          icon: '<svg class="feed-icon" viewBox="0 0 512 512" width="14" height="14"><rect width="512" height="512" fill="#cb3837" rx="50"/><path d="M227 327V185h57v142h57V128H100v256h127v-57z" fill="#fff"/></svg>',
        };
      case "Java":
        return {
          name: "Maven",
          icon: '<svg class="feed-icon" viewBox="0 0 512 512" width="14" height="14"><rect width="512" height="512" fill="#c71a36" rx="50"/><text x="256" y="350" text-anchor="middle" fill="#fff" font-size="200" font-family="serif" font-weight="bold">M</text></svg>',
        };
      case "Go":
        return {
          name: "GitHub",
          icon: '<svg class="feed-icon" viewBox="0 0 16 16" width="14" height="14"><path fill-rule="evenodd" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z" fill="#333"/></svg>',
        };
      default:
  ...
  function computeCurrentStep(p) {
    ...
    if (!specPrUrl)
      return {
        ...
        action: serviceTeam,
        statusClass: "step-blocked",
      };
    if (apiReady !== "completed")
      return {
        status: "API Spec In Progress",
        action: "Spec PR Reviewer",
        statusClass: "step-inprogress",
      };

    // Private preview: no SDK stages — spec merged means done
    if (isPrivatePreview)
      return { status: "Completed", action: "", statusClass: "step-released" };
    ...
    const allMerged = prStatuses.every(
      (s) => s.includes("merged") || s.includes("completed"),
    );
    ...
    const releaseStatuses = activeLangs.map((k) =>
    ...
    return {
      status: "SDK Review In Progress",
      action: "SDK PR Reviewer",
      statusClass: "step-inprogress",
    };
  }
    ...
    for (const [, group] of groups) {
      ...
      const withPr = group.filter((p) => {
        ...
        return Object.values(langs).some(
          (l) => l.sdkPrUrl && !isLangExcluded(l.exclusionStatus),
        );
      });
      ...
      const newOnes = group.filter(
        (p) => p.state === "New" || p.state === "Not Started",
      );
  ...
  function populateMonthFilter(plans) {
    ...
    const months = [...monthSet].sort(
      (a, b) => parseReleaseMonth(a) - parseReleaseMonth(b),
    );
  ...
  function matchesFilter(p, filter) {
    ...
    if (
      p.apiSpec &&
      p.apiSpec.specPrUrl &&
      p.apiSpec.specPrUrl.toLowerCase().includes(filter)
    )
      return true;
    // Package names across all languages
    const langs = p.languages || {};
    for (const lang of Object.keys(langs)) {
      if ((langs[lang].packageName || "").toLowerCase().includes(filter))
        return true;
      if ((langs[lang].sdkPrUrl || "").toLowerCase().includes(filter))
        return true;
    }
    return false;
  }
  ...
  function render(plans) {
    ...
    function splitByState(arr) {
      ...
      const now = new Date();
      const thisMonthKey =
        `${now.toLocaleString("en-US", { month: "long" })} ${now.getFullYear()}`.toLowerCase();
      const lastDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
      const lastMonthKey =
        `${lastDate.toLocaleString("en-US", { month: "long" })} ${lastDate.getFullYear()}`.toLowerCase();
    ...
    const totalInProgress =
      mgmtSplit.inProgress.length + dataSplit.inProgress.length;
  ...
  async function handlePlanRefresh(btn) {
    ...
    try {
      const resp = await fetch(`/api/refresh-plan/${planId}`, {
        method: "POST",
      });
  ...
  function prDetailLabels(l) {
    ...
    const d = l.prDetails;
    const isMerged = (l.sdkPrGitHubStatus || l.prStatus || "")
      .toLowerCase()
      .includes("merged");
    const isClosed =
      (l.sdkPrGitHubStatus || l.prStatus || "").toLowerCase() === "closed";
    const isOpenOrDraft = !isMerged && !isClosed;
    let labels = "";
    if (isOpenOrDraft && d.failedChecks && d.failedChecks.length) {
      const checksUrl = l.sdkPrUrl
        ? l.sdkPrUrl.replace(/\/$/, "") + "/checks"
        : "";
      const checksLink = checksUrl
        ? ` <a href="${esc(checksUrl)}" target="_blank" rel="noopener" style="font-size:.75rem;">View checks</a>`
        : "";
    ...
    if (isOpenOrDraft && d.mergeable && d.mergeableState === "clean") {
      labels +=
        '<span class="pr-label pr-label-mergeable">Ready to merge</span>';
    }
    return labels;
  }
  ...
  function buildActionPopupContent(type, lang, planId, specPath, prUrl, pkg) {
    ...
    let body = "";
    const agentLink =
      '<a href="https://aka.ms/azsdk/agent" target="_blank" rel="noopener">Azure SDK Tools agent</a>';
    const repoUrls = {
  ...
  function actionRequiredHTML(p) {
      ...
      if (langsWithFailedChecks.length) {
        for (const lang of langsWithFailedChecks) {
          ...
          const repoName = repoMatch
            ? repoMatch[1]
            : "azure-sdk-for-" + lang.toLowerCase();
    ...
    const actionFrom = [];
    {
      ...
      if (
        step.status === "API Spec Not Available" ||
  ...
  function computeStages(p) {
    ...
    const allGenerated =
      activeLangs.length > 0 && langsGenerated.length === activeLangs.length;
    const anyCheckFailure = langsWithPr.some((k) => {
      const d = langs[k].prDetails;
      return d && d.failedChecks && d.failedChecks.length > 0;
    });
    const allPrMerged =
      langsGenerated.length > 0 &&
      activeLangs.every((k) => {
        const l = langs[k];
        const ps = (l.sdkPrGitHubStatus || l.prStatus || "").toLowerCase();
        const rs = (l.releaseStatus || "").toLowerCase();
        return (
          ps.includes("merged") ||
          ps.includes("completed") ||
          rs.includes("completed") ||
          rs.includes("released")
        );
      });
    const releaseStatuses = activeLangs.map((k) =>
      (langs[k].releaseStatus || "").toLowerCase(),
    ...
    stages.push({
      label: "Create Spec PR",
      status: s1Done ? "done" : "current",
      icon: "📝",
    });
    ...
    stages.push({
      ...
      status: s2Done ? "done" : s2Current ? "current" : "upcoming",
      icon: "🔍",
    });
    ...
    if (isPrivatePreview) {
      ...
      stages[1] = {
        label: "API Spec Review",
        status:
          s1Done && specApproved ? "done" : s1Done ? "current" : "upcoming",
        icon: "🔍",
      };
  ...
  function detailHTML(p, options) {
    ...
    if (
      p.typeSpecPath &&
      p.specProjectPath &&
      p.typeSpecPath !== p.specProjectPath
    ) {
    ...
    if (p.submittedBy)
      metaItems.push(
        `<span><strong>Submitted By:</strong> ${esc(p.submittedBy)}</span>`,
      );
    if (p.releaseMonth)
      metaItems.push(
        `<span><strong>Release Month:</strong> ${esc(p.releaseMonth)}</span>`,
      );
    if (p.releaseType)
      metaItems.push(
        `<span><strong>SDK Release Type:</strong> ${esc(p.releaseType)}</span>`,
      );
    if (p.createdDate)
      metaItems.push(
        `<span><strong>Created On:</strong> ${shortDate(p.createdDate)}</span>`,
      );
    if (p.lastActivity)
      metaItems.push(
        `<span><strong>Last Activity:</strong> ${shortDate(p.lastActivity)}</span>`,
      );
    if (metaItems.length)
      html += `<div class="detail-meta-row">${metaItems.join("")}</div>`;
    if (showPmAction && p._pmAction)
      html += `<div class="pm-action"><strong>Possible PM action:</strong> ${p._pmAction}</div>`;
    html += "</div>";
    ...
    if (p.productName || p.productId) {
      ...
      if (p.serviceName)
        html += `<div class="detail-row"><strong>Service Name:</strong> ${esc(p.serviceName)}</div>`;
      if (p.productId) {
      ...
      if (p.productLifecycle)
        html += `<div class="detail-row"><strong>Product Lifecycle:</strong> ${esc(p.productLifecycle)}</div>`;
      if (p.ownerPM)
        html += `<div class="detail-row"><strong>Owner / PM:</strong> ${esc(p.ownerPM)}</div>`;
      html += `</div></div>`;
    }
      ...
      if (isPrivPrev) {
        ...
        <table class="sdk-table"><thead><tr>
          ...
          for (const lang of langKeys) {
            ...
            const feedUrl = isReleased
              ? getPackageFeedUrl(lang, l.packageName, displayVersion, p)
              : "";
            let feedLinkCell = "—";
            if (feedUrl) {
    ...
    // API Spec + Spec Approval — horizontal row
    {
      ...
      if (p.apiSpec) {
        ...
        if (s.specPrUrl)
          specItems.push(
            `<span><strong>Spec PR:</strong> <a href="${esc(s.specPrUrl)}" target="_blank" rel="noopener">PR</a></span>`,
          );
        if (s.apiVersion)
          specItems.push(
            `<span><strong>API Version:</strong> ${esc(s.apiVersion)}</span>`,
          );
      }
      if (p.apiReadiness && p.apiReadiness !== "unknown") {
        const approvalLabel =
          p.apiReadiness === "completed"
            ? "Approved (PR Merged)"
            : "Pending (PR Open)";
        const approvalCls =
          p.apiReadiness === "completed"
            ...
            : "status-inprogress";
        specItems.push(
          `<span><strong>Spec Approval:</strong> <span class="${approvalCls}">${approvalLabel}</span></span>`,
        );
      } else if (p.specApprovalStatus) {
        specItems.push(
          `<span><strong>Spec Approval:</strong> ${statusSpan(p.specApprovalStatus)}</span>`,
        );
      }
      if (specItems.length) {
      ...
      if (p.specPrLabels && p.specPrLabels.length) {
        const labelPills = p.specPrLabels
          .map((label) => {
      ...
      if (
        p.apiSpec &&
        p.apiSpec.previousSpecPrUrls &&
        p.apiSpec.previousSpecPrUrls.length
      ) {
  ...
  async function lazyLoadPrDetails(detailsEl, cardEl) {
    ...
    try {
      ...
      for (const link of prLinks) {
        ...
        if (info.gitHubStatus) {
          ...
          if (plan) {
            ...
            const langData =
              langName && plan.languages && plan.languages[langName];
        ...
        if (info.prDetails) {
          ...
          let labels = "";
          const lazyMerged = (info.gitHubStatus || "")
            .toLowerCase()
            .includes("merged");
          const lazyClosed =
            (info.gitHubStatus || "").toLowerCase() === "closed";
          const lazyOpenOrDraft = !lazyMerged && !lazyClosed;
          if (lazyOpenOrDraft && d.failedChecks && d.failedChecks.length) {
            const checksUrl = link.href
              ? link.href.replace(/\/$/, "") + "/checks"
              : "";
            const checksLink = checksUrl
              ? ` <a href="${esc(checksUrl)}" target="_blank" rel="noopener" style="font-size:.75rem;">View checks</a>`
              : "";
      ...
      if (plan && cardEl) {
        ...
        const isInPmTab =
          currentUserIsPM && !!(cardEl && cardEl.closest("#tab-pm-view"));
        if (isInPmTab && plan._pmAction) {
          ...
          if (oldAction) {
            ...
            const insertBefore =
              detailsEl.querySelector(".action-note") ||
              detailsEl.querySelector(".detail-agent-link");
            if (insertBefore)
  ...
  async function lazyLoadPreviousSdkPrs(detailsEl, planId) {
    const container = detailsEl.querySelector(
      `.previous-sdk-prs[data-plan-id="${planId}"]`,
    );
  ...
  document.addEventListener("click", (e) => {
    ...
    if (actionBtn) {
      e.stopPropagation();
      const { actionType, lang, planId, specPath, prUrl, pkg } =
        actionBtn.dataset;
      const { title, body } = buildActionPopupContent(
        actionType,
        lang,
        planId,
        specPath,
  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.effect(() => {
      ...
      searchTimeout = setTimeout(() => {
        ...
        if (/^\d+$/.test(filter)) {
          const found = getPlans().some(
            (p) =>
              String(p.releasePlanId) === filter || String(p.id) === filter,
          );
          if (!found) {
            fetch(
              `/api/release-plans?releasePlan=${encodeURIComponent(filter)}`,
            )
  ...
  function renderPMView(plans) {
    ...
    for (const p of filtered) {
      ...
      if (p.releaseMonth) {
        const target = parseReleaseMonth(p.releaseMonth);
        if (
          target.getFullYear() !== 9999 &&
          target >= thisMonth &&
          target <= endOfNextMonth
        ) {
      ...
      if (isPartial && !missing) {
        p._pmAction =
          "Reach out to service team and confirm the plan to release remaining languages.";
        partial.push(p);
      }
    }
  ...
  function renderPMSection(listId, plans) {
    ...
    el.innerHTML = plans
      .map((p) => cardHTML(p, { showPmAction: true }))
      .join("");
  }
  ...
  function extractCandidatePRs(plans) {
    ...
    for (const p of plans) {
      ...
      for (const [lang, l] of Object.entries(p.languages)) {
        ...
        seen.add(dedupeKey);
        const prMatch = l.sdkPrUrl.match(
          /github\.com\/([^/]+\/[^/]+)\/pull\/(\d+)/,
        );
  ...
  async function progressiveLoadPRStatuses(plans) {
    ...
    for (let i = 0; i < uniqueUrls.length; i += BATCH) {
      ...
      if (prLoading)
        prLoading.querySelector("p").textContent =
          `Fetching PR statuses (${Math.min(fetched, uniqueUrls.length)}/${uniqueUrls.length})…`;
  ...
  function prStatusBadge(status) {
    ...
    if (st.includes("merged"))
      return '<span class="badge badge-finished">Merged</span>';
    if (st === "draft") return '<span class="badge badge-new">Draft</span>';
    if (st === "open")
      return '<span class="badge badge-inprogress">Open</span>';
    if (st === "closed")
      return '<span class="badge badge-pastdue">Closed</span>';
  ...
  function prDetailHTML(pr, info) {
    ...
    if (title)
      html += `<div class="pr-detail-row"><strong>Title:</strong> ${esc(title)}</div>`;
    ...
    if (pr.releaseStatus)
      html += `<div class="pr-detail-row"><strong>Release Status:</strong> ${statusSpan(pr.releaseStatus)}</div>`;
    ...
    if (info && info.prDetails) {
      ...
      if (prIsOpenOrDraft && d.failedChecks && d.failedChecks.length) {
        const checksUrl = pr.prUrl
          ? pr.prUrl.replace(/\/$/, "") + "/checks"
          : "";
        const checksLink = checksUrl
          ? ` <a href="${esc(checksUrl)}" target="_blank" rel="noopener">View checks →</a>`
          : "";
  ...
  function renderFilteredPRs() {
    ...
    if (!filtered.length) {
      container.innerHTML =
        '<p style="padding:8px;color:#605e5c;font-size:.88rem;">No pull requests found.</p>';
      return;
    }
  ...
  async function lazyLoadPrCardDetails(cardEl) {
    ...
    try {
      ...
      if (!res.ok) {
        detailsEl.innerHTML =
          '<div class="pr-detail-loading" style="color:var(--red);">Failed to load details.</div>';
        return;
      }
      const data = await res.json();
          ...
          if (pr.prDetails && pr.prDetails.isApproved && !existingApproved) {
            ...
            if (statusEl)
              statusEl.insertAdjacentHTML(
                "afterend",
                ' <span class="pr-label pr-label-approved">Approved</span>',
              );
          } else if (
            existingApproved &&
            !(pr.prDetails && pr.prDetails.isApproved)
          ) {
      ...
      detailsEl.innerHTML =
        '<div class="pr-detail-loading" style="color:var(--red);">Error loading details.</div>';
    }
  }
